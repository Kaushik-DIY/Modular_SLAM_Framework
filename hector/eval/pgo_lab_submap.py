...
def _build_mini_submap(
    ...
    """
    Seed a temporary Submap with scans around `center_idx`.
    """
    ...
    for idx in range(lo, hi + 1):
        ...
        if pts.shape[0] == 0:
            continue
        p = Pose2(float(pose_arr[0]), float(pose_arr[1]), float(pose_arr[2]))
        # insert_scan expects points in sensor local frame
        builder.insert_scan(p, pts)
...
def detect_loop_closures(
    ...
    for j in kf_indices:
        ...
        for i in int_arr(candidates):
            # Build a mini submap seeded around node i
            ...
            # Decimate if too many points to match submap config of 200 max points
            if n_pts > 200:
                np.random.seed(j)
                indices = np.random.choice(n_pts, 200, replace=False)
                sub_pts = pts_j[indices]
            else:
                sub_pts = pts_j

            predicted = _pose2_from_arr(poses[i])  # initialise at anchor pose

            grid_prob = mini_submap.grid.probability()
            best_pose, score = correlative_match_two_stage(
                ...
                min_valid=20,
                coarse_xy_window=0.5,
                coarse_th_window=np.deg2rad(15.0),
                coarse_xy_step=0.15,
                coarse_th_step=np.deg2rad(4.0)
            )
            ...
            if score >= lc_min_score:
                ...
                x0 = np.array([best_pose.x, best_pose.y, best_pose.theta])
                x_opt = solver.solve(x0, problem.compute_r_J)
                refined_pose_arr = x_opt.reshape(3)

                matched_arr = _se2_arr(refined_pose_arr[0], refined_pose_arr[1], refined_pose_arr[2])
                z_ij = _se2_between(poses[i], matched_arr)
                closures.append(LoopClosure(i=i, j=j, z_ij=z_ij, score=score))
                print(f"[pgo_lab]   LC accepted: ({i},{j})  score={score:.3f}  "
...
def rebuild_map_from_poses(
    ...
    for pose_arr, pts in zip(poses_opt, scan_pts_list):
        if pts.shape[0] == 0:
            continue
        p = Pose2(float(pose_arr[0]), float(pose_arr[1]), float(pose_arr[2]))
        # Note: PTS requires strictly local frame points! SubmapBuilder handles conversion securely 
        builder.insert_scan(p, pts)

    # Master reconstruction map
...
def main() -> None:
    ...
    ap.add_argument("--lc_min_score", type=float, default=0.70,
    ...
    prob_after = grid_after.probability().astype(np.float32)
    ...
    prob_before = grid_before.probability().astype(np.float32)
    ...
    print(f"[pgo_lab] PGO map stats: {occ} occupied, {free} free, "
          f"{total - occ - free} unknown  ({args.map_res}m res, {grid_after.w}x{grid_after.h})")
