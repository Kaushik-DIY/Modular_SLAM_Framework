...
rebuild_map_lab.py
==================
Post-run occupancy-map reconstruction for the lab_run_2 dataset.
...
def build_map(
    ...
    l_max: float = 5.0,
    label: str = "",
) -> tuple[GridMap, np.ndarray]:
    """
    Replay scan points into a fresh GridMap at the given poses.

    Returns
    -------
    grid      : GridMap — fully integrated occupancy grid
    traj_xy   : (N, 2) ndarray — positions used for trajectory overlay
    """
    ...
    for i, (pose_row, pts) in enumerate(zip(poses_xyt[:N], pts_list[:N])):
        ...
        pose_arr = np.array([x, y, th], dtype=float)
        traj_xy.append([x, y])
...
def _render_diff_panel(
    ...
    def _overlay(traj_xy, color, label, marker):
        if traj_xy.shape[0] == 0:
            return
        gxy = grid_ref.world_to_grid(traj_xy)
        px, py = gxy[:, 0], grid_ref.size - 1 - gxy[:, 1]
    ...
    for i in range(0, N, stride):
        g_loc = grid_ref.world_to_grid(traj_local[i:i+1])[0]
        g_pgo = grid_ref.world_to_grid(traj_pgo[i:i+1])[0]
        dx = float(g_pgo[0] - g_loc[0])
        dy = float((grid_ref.size - 1 - g_pgo[1]) - (grid_ref.size - 1 - g_loc[1]))
        mag = float(np.hypot(dx, dy))
        if mag < 0.5:
            continue
        ax.annotate(
            "",
            xy=(float(g_pgo[0]), float(grid_ref.size - 1 - g_pgo[1])),
            xytext=(float(g_loc[0]), float(grid_ref.size - 1 - g_loc[1])),
...
def save_comparison_png(
    ...
    fig.suptitle(
        f"Lab Room — Hector SLAM  |  Local SLAM vs PGO{lc_str}",
...
def _latest_pgo_traj(out_dir: str, variant: str) -> Optional[str]:
    p = Path(out_dir)
    candidates = sorted(
        p.glob(f"trajectory_lab_run_2_{variant}_scan_to_map_*_pgo.txt"),
        key=lambda f: f.stat().st_mtime,
        reverse=True,
    )
...
def main() -> None:
    ...
    prob_local = grid_local.prob().astype(np.float32)
    ...
    _print_stats(prob_local, args.map_res, grid_local.size, label="Local")
    ...
    if poses_pgo is not None:
        ...
        prob_pgo = grid_pgo.prob().astype(np.float32)
        ...
        _print_stats(prob_pgo, args.map_res, grid_pgo.size, label="PGO")
